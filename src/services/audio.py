...
class AudioService(AudioServiceAbs):
    def __init__(self, sample_rate: int = 16000, language: str = 'fr', model_size: str = 'base'):
        self.model = WhisperModel(
            model_size, compute_type="int8", device="cpu")
        if sample_rate != 16000:
            raise ValueError(
                "Whisper model requires audio with a sample rate of 16000 Hz.")
    ...
    def transcribe(self) -> str:
        ...
        audio_np = np.frombuffer(
            self.buffer, dtype=np.int16).astype(np.float32) / 32768.0
